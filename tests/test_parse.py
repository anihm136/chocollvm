    ...
    def test_parse(self):
        for file in self.get_test_files(
            ...
            with self.subTest(file=file):
                ...
                # if filename starts with 'bad', expect errors
                if file.name.startswith("bad"):
                    self.assertTrue(len(astparser.errors) > 0)
                else:
                    ast_json = ast.toJSON(dump_location=True)
                    try:
                        with file.with_suffix(".py.ast").open("r") as f:
                            correct_json = json.load(f)
                            self.assertTrue(ast_equals(ast_json, correct_json))
                    except:
                        with file.with_suffix(".py.ast.typed").open("r") as f:
                            correct_json = json.load(f)
                            self.assertTrue(ast_equals(correct_json, ast_json))
        for file in self.get_test_files(
            [
                self.tc_test_dir,
            ]
